...
docker_install() {
    ...
    if [ -x "$(command -v docker)" ]; then
        ...
        if [ $lsb_dist == "openwrt" ]; then
            ...
            echo "安装 docker 环境..."
            bash <(curl -sSL https://gh.llkk.cc/https://raw.githubusercontent.com/yanyuwangluo/VIP/main/Scripts/sh/docker.sh)  --install-latested true --source https://mirrors.aliyun.com/docker-ce --source-registry https://registry.cn-hangzhou.aliyuncs.com --close-firewall 
...
if [ "$access" != "2" ]; then
    if [ "$(grep -c "token" $DATA_PATH/config/auth.json)" != 0 ]; then
        log "8.开始青龙内部配置"
        docker exec -it $CONTAINER_NAME bash -c "$(curl -fsSL https://gh.llkk.cc/https://raw.githubusercontent.com/yanyuwangluo/VIP/main/Scripts/sh/1customCDNN.sh)"
    else
